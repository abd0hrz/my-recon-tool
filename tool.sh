...
#--------------------------------------Get Live Subdomains-------------------------------
        ...
        cat Recon/$1/tmp_for_subs.txt | httpx -silent -ports 80,443,8080,8443,9443,9080,8888,3000,81,30001,9000,10000 | cut -d '/' -f 3 | sort -u >> Recon/$1/live-subs.txt;
...
#----------------------------------Subdomain TakeOver-------------------------------------------------------
        ...
        then
                ...
                subzy --hide_fails -targets Recon/$1/subdomains.txt;
                printf "\n\n";
                # subzy --hide_fails -targets Recon/$1/ips.txt;
        else
...
#-----------------------------------Get Parameters------------------------------------------------------------------------
        ...
        if [ -x "$(command -v waybackurls)" ]
        then
                cat Recon/$1/live-subs.txt | waybackurls | sed -e 's/:80//' | grep "?[a-z0-9]*=" | sort -u >> Recon/$1/parameters;
        else
                printf "${RED}[-]----------Waybackurls Not Found----------[-]\n\n";
                printf "\n${WHITE} You can Download it  [ go install github.com/tomnomnom/waybackurls@latest ]\n\n";
        fi
...
printf "${YELLOW} [i] Syntax  ./recon.sh domain.com  subdonains-wordlist.txt"
fi
